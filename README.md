...
При установке через npm стоит сразу задать `CONFIGS_DIR`/`RESULTS_DIR` в `.env` на путь **снаружи** `node_modules` — эту папку принято сносить и переустанавливать не задумываясь, а вместе с ней потерялись бы все отчёты и вся история выгрузок.
